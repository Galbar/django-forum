...
class SpoilerTag(TagBase):
    """ By Alessio Linares """
    def __init__(self, name, **kwargs):
        super(SpoilerTag, self).__init__(name, strip_first_newline=True)

    def render_open(self, parser, node_index):
        return u'<div><button type="button" class="btn btn-default btn-xs spoiler-btn" >Spoiler</button>\
...
class YoutubeTag(TagBase):
    ...
    def render_open(self, parser, node_index):
        ...
        # Validate code
        if self.params:
            code = self.params.strip()
        else:
            code = strip_bbcode(contents)
            
        return u"""<div class="h_iframe">
        <!-- a transparent image is preferable -->
        <img class="ratio" src="http://placehold.it/16x9"/>
        <iframe src="http://www.youtube.com/embed/%s" frameborder="0" allowfullscreen></iframe>
    </div>""" % PostMarkup.standard_replace_no_break(code)
...
def create(include=None,
    ...
    add_tag(YoutubeTag, u'youtube')
    add_tag(SpoilerTag, u'spoiler')
